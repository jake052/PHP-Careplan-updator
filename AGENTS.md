...
- **Guard:** `eventsUrl` is restricted to loopback hosts (localhost / 127.0.0.1 / ::1) — dev-only SSRF guard. Production would replace this with a typed RPC into a real event store.
